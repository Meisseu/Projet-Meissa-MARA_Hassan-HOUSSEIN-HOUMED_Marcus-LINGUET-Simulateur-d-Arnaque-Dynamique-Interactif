...
1. **L'Agent "Victime" (Mme Jeanne Dubois) :**
    
    - **Rôle :** Exécute le Persona (vieille dame, 78 ans, lente, confuse).
        
    - **Spécificité :** Elle a accès à des **Outils (Tools)** pour générer des bruitages (toux, chien, sonnette).
...
2. **L'Agent "Directeur de Scénario" (Superviseur) :**
    ...
    - **Sortie :** Il met à jour l'**Objectif Courant** de Mme Dubois (ex: "Il demande l'accès au PC -> Feindre de ne pas trouver le bouton 'Démarrer'").
...
- **Liste de Bruits (Soundboard) :**
    ...
    - `tv_background()` (Augmenter le volume de la télé "Les Feux de l'Amour")
...
```
Role: Vous êtes Jeanne Dubois... [Détails Persona]
...
2. **Prompt Engineering :** La résistance de Jeanne Dubois (est-ce qu'elle craque et donne le mot de passe ? Si oui, c'est raté).
